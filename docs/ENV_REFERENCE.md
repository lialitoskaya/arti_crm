...
## TOTP для администраторов

```env
CRM_TOTP_ENCRYPTION_KEY=
```

Отдельный Fernet-ключ для шифрования TOTP-секретов. Значение должно быть корректным
URL-safe base64 Fernet key, одинаковым на всех worker-процессах и настроенным до
первого подключения двухфакторной аутентификации. Fallback на `SECRET_KEY`, CSRF,
VAPID или другие ключи отсутствует. Ключ нельзя логировать, передавать в браузер или
менять, пока существует хотя бы одна включённая TOTP-запись.

Без переменной пользователи без включённого TOTP входят по паролю как раньше.
Подключение, отключение и вход пользователя с уже включённым TOTP завершаются
безопасной общей ошибкой и не создают полноценную сессию. Для корректной проверки
30-секундных кодов системное время всех worker-процессов должно быть синхронизировано.
