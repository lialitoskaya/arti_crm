...
Диалог материализуется в CRM только после валидного непустого ответа chat history. Пустой
список сообщений является успешным ответом, но не создаёт новый chat shell. Ошибка запроса,
отсутствующее поле `messages` или неверный тип ответа считаются ошибкой синхронизации, а не
пустой историей.

Создание нового диалога и сохранение всей первой истории выполняются одной SQLite-транзакцией
`BEGIN IMMEDIATE`. Ошибка первого или последующего сообщения откатывает новый диалог целиком.
Ранее созданные legacy shells не удаляются: обычный list sync повторно запрашивает их историю
один раз за проход; пустая история не меняет техническую активность, а непустая заполняет shell
атомарно.

В списке диалогов Yandex shells без сохранённых сообщений скрываются через `EXISTS`, но остаются
в БД вместе с назначениями и задачами. Видимые диалоги сортируются только по времени фактически
сохранённого последнего сообщения и затем по id. `chats.updated_at` отражает изменение данных
диалога и не является временем пользовательской активности; идентичный sync и no-op repair его
не обновляют.
